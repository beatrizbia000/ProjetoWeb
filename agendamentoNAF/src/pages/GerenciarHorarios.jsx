...
export default function GerenciarHorarios() {
  ...
  const carregarHorarios = async () => {
    try {
      const response = await axios.get(`${API_URL}/agendamentos/horarios-disponiveis`);
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm">
          ...
          {horarios.length === 0 ? (
            ...
            <ul className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {horarios.map((h) => (
                <li key={h.id} className="p-3 border rounded bg-gray-50 flex justify-between items-center">
                  <span>
                    {new Date(h.data_horario).toLocaleDateString('pt-BR')} - {new Date(h.data_horario).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'})}
                  </span>
                </li>
              ))}
            </ul>
